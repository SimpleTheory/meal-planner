...
class ShoppingListState {
  ...
  ShoppingListState(this.diet, {required this.shoppingList, required this.selected});

  factory ShoppingListState.init(Diet diet){
    diet.updateShoppingList();
    final shoppingList = diet.shoppingList.entries.toList();
    return ShoppingListState(diet, shoppingList: shoppingList, selected: <MealComponent>[]);
  }
  ...
  ShoppingListState copyWith({
    Diet? diet_, List<
        MapEntry<String, List<MealComponent>>>? shoppingList_, List<
        MealComponent>? selected_,}) {
  ...
  void moveItemsInSelected(String target){
    for (MapEntry<String, List<MealComponent>> entry in diet.shoppingList.entries){
      diet.shoppingList[entry.key] = entry.value.where((element) => !(selected.contains(element))).toList();
    }
